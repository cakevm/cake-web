...
use crate::model::{Filter, QuoteRequest, QuoteResponse};
use alloy_primitives::Address;
use axum::extract::{Path, Query, State};
use axum::http::StatusCode;
use axum::Json;
use cake_web_dto::model::Pagination;
    ...
    match app_state.bc.market().read().await.pools().get(&address) {
        ...
        Some(pool) => {
            ...
            match quote_result {
                ...
                Ok((out_amount, gas_used)) => Ok(Json(crate::model::QuoteResponse { out_amount, gas_used })),
            }
        }
    }
}
